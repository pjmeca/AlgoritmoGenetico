...
void crear_imagen(const RGB *imagen_objetivo, int num_pixels, int ancho, int alto, int max, int num_generaciones, int tam_poblacion, RGB *imagen_resultado, const char *output_file, int nh_for_ini, int nh_for_fit)
{
	...
	double ti = mseconds();
	#pragma omp parallel for schedule(dynamic, 4) num_threads(nh_for_ini)
	for (i = 0; i < tam_poblacion; i++)
	{
		poblacion[i] = (Individuo *)malloc(sizeof(Individuo));
		poblacion[i]->imagen = imagen_aleatoria(max, num_pixels);
		fitness(imagen_objetivo, poblacion[i], num_pixels, nh_for_fit);
	}
	double tf = mseconds();
	printf("FOR_INI con %d hilos = %.6lf mseconds\n", nh_for_ini, (tf - ti));
...
void fitness(const RGB *objetivo, Individuo *individuo, int num_pixels, int nh_for_fit)
{
	...
	double fitness = 0.0;
	//#pragma omp parallel for reduction(+: fitness) num_threads(nh_for_fit)
	for (int i = 0; i < num_pixels; i++)
	{
		// Distancia euclídea
